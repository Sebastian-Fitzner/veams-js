...
var $ = require('jquery');
var App = require('../../app');
var Pagination = require('./ui-carousel-pagination');
 ...
 */
App.ui.Carousel = App.ComponentView.extend({
	...
	_buildCarousel: function() {
		if (this.options.responsive) {
			if (!this.$el.find('[data-js-atom="carousel-list-group"]').length) {
	...
	setCurrentSlideIndex: function(opts) { // When we are changing the URL go to the specific index
		...
		this.currentSlideIndex = opts.direction === 'next' ? this.getNextSlide(this.currentSlideIndex) : this.getPrevSlide(this.currentSlideIndex);
	},

	/**
	 * get previous slide index for given slide index
	 * @param {number} slideIndex
	 * @returns {number}
	 */
	getPrevSlide: function(slideIndex) {
		var prevSlide;

		if (slideIndex - 1 <= 0) {
			prevSlide = this.slideLength;
		} else {
	...
	/**
	 * get next slide index for given slide index
	 * @param {number} slideIndex
	 * @returns {number}
	 */
	getNextSlide: function(slideIndex) {
		var nextSlide;

		if (slideIndex + 1 > this.slideLength) {
			nextSlide = 1;
		} else {
	...
	render: function() {
		var that = this;
		that._buildCarousel();
		that._hidePaginationByOneSlide();
		that.getFirstSlide();
		that.getAndSetSlideDimensions();
		that._bindEvents();
		that.initPagination();

		that.$el.removeClass(that.options.unresolvedClass);
...
module.exports = App.ui.Carousel;
